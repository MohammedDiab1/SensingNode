...
bool enableSwitched = true; //modified fom false to true
bool enabled = true;
...
//Diab Modifications
std::vector<geometry_msgs::Pose> Poses; //create a vector of tags that attached with the objects
bool enablecam=true;

void getCapCallback (const sensor_msgs::ImageConstPtr & image_msg);
std::vector<Pose> getCapCallback1 (const sensor_msgs::ImageConstPtr & image_msg);
...
void getCapCallback (const sensor_msgs::ImageConstPtr & image_msg)
{
    //If we've already gotten the cam info, then go ahead
        if(cam->getCamInfo_){
            ...
            //Diab Modifications
            Poses.resize(marker_detector.markers->size());
                        ...
                        int id = (*(marker_detector.markers))[i].GetId();
                                ...
                                //Diab Modifications
                                Poses[i].position.x=px;
                                Poses[i].position.y=py;
                                Poses[i].position.z=pz;
                                Poses[i].orientation.x=qx;
                                Poses[i].orientation.y=qy;
                                Poses[i].orientation.z=qz;
                                Poses[i].orientation.w=qw;

  //std::cout<<"Position of Marker "<<i<<" is "<<px<<" "<<py<<" "<<pz<<std::endl;
...
//Modifications to make a services

std::vector<Pose> getCapCallback1 (const sensor_msgs::ImageConstPtr & image_msg)
{
    std::vector<Pose> tagpose;
    //If we've already gotten the cam info, then go ahead
    if(cam->getCamInfo_){
        try{
            tf::StampedTransform CamToOutput;
...
bool srvLocateObjects(sensing_node::LocateObjects::Request &req,
             sensing_node::LocateObjects::Response &res)
{
...
bool srvcameraStatus(sensing_node::cameraStatus::Request &req,
             sensing_node::cameraStatus::Response &res)
{
    enableSwitched=req.enable;
    enabled=req.enable;
    res.status=true;
    return true;
}
...
int main(int argc, char *argv[])
{
  ...
  enableSwitched = true;
    ros::ServiceServer service1 = n.advertiseService("sensing_node/LocateObjects",srvLocateObjects);
    ros::ServiceServer service2 = n.advertiseService("sensing_node/cameraStatus",srvcameraStatus);
